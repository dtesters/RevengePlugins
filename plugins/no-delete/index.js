// NoDelete (rewritten) following trainingdummy/vendetta-plugins format
// https://github.com/trainingdummy/vendetta-plugins

const vendettaGlobal = (typeof globalThis !== "undefined" && globalThis.vendetta)
  ? globalThis.vendetta
...
if (!storage.uiMessageIds) storage.uiMessageIds = {};
...
function injectLocalLogMessage(channelId, content, embedsText) {
  ...
  if (embedsText) message.content += `\n\nEmbeds:\n${embedsText}`;
  try { MessageActions.receiveMessage(channelId, message); } catch {}
  try { ensureChannelUiIds(channelId).push(id); } catch {}
}
function onMessageDeleted(payload) {
  ...
  ensureChannelLog(channelId).push({
    type: "delete",
    messageId,
    channelId,
    author,
                ...
                onPress: () => {
                  try {
                    ...
                    if (channelId) {
                      ...
                      for (const id of ids) {
                        try { FluxDispatcher.dispatch({ type: "MESSAGE_DELETE", channelId, id }); } catch {}
                      }
                      storage.uiMessageIds[channelId] = [];
                    }
                    if (typeof showToast === "function") showToast("Cleared NoDelete log", 1);
                  } catch {}
